...
(defvar poly-point-in-comment-functions ()
...
function to return non-nil. Used by `poly-point-in-comment-p'.")

(defvar poly-point-in-string-functions ()
  "List of functions to run to determine if point is in a string.
Each function takes one argument: the position of the point. Stops on the first
function to return non-nil. Used by `poly-point-in-string-p'.")

;;;###autoload
(defun poly-surrounded-p (pair &optional inline balanced)
...
;;;###autoload
(defun poly-point-in-comment-p (&optional pos)
  "Return non-nil if POS is in a comment.
POS defaults to the current position."
  (let ((pos (or pos (point))))
    (or (run-hook-with-args-until-success 'poly-point-in-comment-functions pos)
        (sp-point-in-comment pos))))

;;;###autoload
(defun poly-point-in-string-p (&optional pos)
  "Return non-nil if POS is in a string."
  ;; REVIEW Should we cache `syntax-ppss'?
  (let ((pos (or pos (point))))
    (or (run-hook-with-args-until-success 'poly-point-in-string-functions pos)
        (sp-point-in-string pos))))

;;;###autoload
(defun poly-point-in-string-or-comment-p (&optional pos)
  "Return non-nil if POS is in a string or comment."
  (or (poly-point-in-string-p pos)
      (poly-point-in-comment-p pos)))
...
;;;###autoload
(defun poly-thing-at-point-or-region (&optional thing prompt)
...
in some cases."
  ...
  (cond ((stringp thing)
         thing)
        ((poly-region-active-p)
         (buffer-substring-no-properties
          (poly-region-beginning)
          (poly-region-end)))
        (thing
...
(defun poly--bol-bot-eot-eol (&optional pos)
    ...
    (let* ((bol (if visual-line-mode
           ...
           (eot (or (save-excursion
                      (if (not comment-use-syntax)
                        ...
                        (goto-char eol)
                        (while (and (poly-point-in-comment-p)
                                    (> (point) bol))
                          (backward-char))
...
(defvar poly--last-backward-pt nil)
;;;###autoload
(defun poly/backward-to-bol-or-indent (&optional point)
  "Jump between the indentation column (first non-whitespace character) and the
beginning of the line. The opposite of
`poly/forward-to-last-non-comment-or-eol'."
  (interactive "^d")
  (let ((pt (or point (point))))
    (cl-destructuring-bind (bol bot _eot _eol)
        (poly--bol-bot-eot-eol pt)
      (cond ((> pt bot)
             (goto-char bot))
            ((= pt bol)
             (or (and poly--last-backward-pt
                      (= (line-number-at-pos poly--last-backward-pt)
                         (line-number-at-pos pt)))
                 (setq poly--last-backward-pt nil))
             (goto-char (or poly--last-backward-pt bot))
             (setq poly--last-backward-pt nil))
            ((<= pt bot)
             (setq poly--last-backward-pt pt)
             (goto-char bol))))))

(defvar poly--last-forward-pt nil)
;;;###autoload
(defun poly/forward-to-last-non-comment-or-eol (&optional point)
  "Jumps between the last non-blank, non-comment character in the line and the
true end of the line. The opposite of `poly/backward-to-bol-or-indent'."
  (interactive "^d")
  (let ((pt (or point (point))))
    (cl-destructuring-bind (_bol _bot eot eol)
      ...
      (cond ((< pt eot)
             (goto-char eot))
            ((= pt eol)
             (goto-char (or poly--last-forward-pt eot))
             (setq poly--last-forward-pt nil))
            ((>= pt eot)
             (setq poly--last-backward-pt pt)
             (goto-char eol))))))

;;;###autoload
(defun poly/backward-kill-to-bol-and-indent ()
...
;;;###autoload
(defun poly/delete-backward-word (arg)
...
;;;###autoload
(defun poly/retab (arg &optional beg end)
...
;;;###autoload
(defun poly/delete-trailing-newlines ()
...
;;;###autoload
(defun poly/toggle-indent-style ()
...
;;;###autoload
(defun poly/set-indent-width (width)
  ...
  (cond ((require 'editorconfig nil t)
        ...
        ((require 'dtrt-indent nil t)
         (when-let (var (nth 2 (assq major-mode dtrt-indent-hook-mapping-list)))
           (poly-log "Updated %s = %d" var width)
           (set var width))))
